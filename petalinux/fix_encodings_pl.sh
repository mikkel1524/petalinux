...
echo "Rozpoczynam skanowanie plikow tekstowych w bieZZcym katalogu i podkatalogach..."
...
find . -type f -print0 | while IFS= read -r -d $'\0' file; do
    ...
    if file "$file" | grep -q 'text'; then
        ...
        if grep -q "[ZZZZZoZZZZZZZZÓZZZ]" "$file"; then
            echo "Znaleziono polskie litery w pliku: $file"
            # Utworz tymczasowy plik do zapisu zmodyfikowanej zawartoZci
            temp_file=$(mktemp)

            # Iteruj przez mapowanie i zamieniaj litery za pomocZ sed
            # sed wymaga podwojnych cudzysZowow dla zmiennych, aby bash je rozwinZZ
